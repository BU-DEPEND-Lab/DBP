...
#include "footbot_flocking_OSP_byzantine.h"
...
void CFootBotFlockingOSP_Byzantine::SWheelTurningParams::Init(TConfigurationNode &t_node)
{
   try
   {
...
void CFootBotFlockingOSP_Byzantine::SFlockingInteractionParams::Init(TConfigurationNode &t_node)
{
   try
   {
...
void CFootBotFlockingOSP_Byzantine::SByzantineParams::Init(TConfigurationNode &t_node)
{
   try
   {
      Real x, y;
 ...
 */
Real CFootBotFlockingOSP_Byzantine::SFlockingInteractionParams::GeneralizedLennardJones(Real f_distance)
{
...
CFootBotFlockingOSP_Byzantine::CFootBotFlockingOSP_Byzantine() : m_pcWheels(NULL),
                                                                 m_pcLight(NULL),
                                                                 m_pcLEDs(NULL),
                                                                 m_pcCamera(NULL),
                                                                 m_pcPosition(NULL) {}
...
void CFootBotFlockingOSP_Byzantine::Init(TConfigurationNode &t_node)
{
   /*
...
void CFootBotFlockingOSP_Byzantine::ControlStep()
{
...
void CFootBotFlockingOSP_Byzantine::Reset()
{
   ...
   x.emplace(m_sByzantineParams.target);
   current_time = 0;
}
...
CVector2 CFootBotFlockingOSP_Byzantine::VectorToLight()
{
   // /* Get light readings */
...
CVector2 CFootBotFlockingOSP_Byzantine::FlockingVector()
{
...
void CFootBotFlockingOSP_Byzantine::SetWheelSpeedsFromVector(const CVector2 &c_heading)
{
...
void CFootBotFlockingOSP_Byzantine::SetBroadcastFromLinearUpdate()
{
   CVector2 p_self;
   m_pcPosition->GetReading().Position.ProjectOntoXY(p_self);
   ...
   CByteArray cMessage;
   OSP_Message OSP_msg = OSP_Message(FlockingObservationMessage(
                                         x.value(),
                                         x.value()),
                                     GetId(), current_time, 0);
   OSP_msg.serialize(&cMessage);
   // auto new_accusation = OSP_Message(AccusationMessage("fb0"), GetId(), current_time, 0);
   // cMessage << dynamic_cast<Message *>(&new_accusation);
 ...
 */
REGISTER_CONTROLLER(CFootBotFlockingOSP_Byzantine, "footbot_flocking_OSP_byzantine_controller")
